...
bool StarDetector::detect(const std::vector<uint16_t>& image, int width, int height,
    ...
    if (options.spatiallyBalanced && !candidates.empty()) {
        ...
        if (selectedCandidates.size() > options.maxCandidates) {
            ...
            size_t budget = options.maxCandidates;
            ...
            } else if (budget < nonEmptyCount) {
                // 预算少于非空网格数：均匀步长采样，确保每个被选网格不同
                for (size_t i = 0; i < budget; ++i) {
                    size_t pos = static_cast<size_t>((i + 0.5) * nonEmptyCount / budget);
                    if (pos >= nonEmptyCount) pos = nonEmptyCount - 1;
                    size_t idx = nonEmptyIndices[pos];
                    result.push_back(grid[idx][0]);
                }
                selectedCandidates = std::move(result);
            } else {
                // 预算 >= 非空网格数：先从每格取 1 个，再 round-robin 补充
                std::vector<size_t> indices(grid.size(), 0);
                // 第一轮：每格取最强 1 个
                for (size_t idx : nonEmptyIndices) {
                    result.push_back(grid[idx][0]);
                    indices[idx] = 1;
                }
                // 剩余预算用 round-robin
                bool progress = true;
                while (result.size() < budget && progress) {
                    progress = false;
                    for (size_t i = 0; i < grid.size() && result.size() < budget; ++i) {
