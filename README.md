...
> **当前开发版本**：v0.11.0。桌面界面重构为单窗口三栏工作台，新增场景素材栏、统一检查器、内容级视图状态、主窗口任务进度、素材撤销、处理报告与最近结果只读预览。
...
- Average / Median / Kappa-Sigma / Winsorized 堆栈；RGB 像素以整组三通道判断有效性，稳健算法共享同一亮度剔除集合，避免通道独立裁剪拼出紫色或青色伪边
...
- 自动镜头横向色差校正：从参考帧未饱和星点估计红、蓝相对绿色通道的径向尺度；只有星点数量、画面分布和残差同时通过门限才启用，并与几何对齐合并为一次重采样
...
- 可独立启用的星点去紫边：比较星核与星翼色度，只校正向外突增的彩色边，不改变星点尺寸；缩星继续使用局部 RGB 背景重建的 Starless/Stars 分离、亚像素圆形 Minimum 和弱残留清理
...
- 面向六类拍摄任务的单窗口三栏工作台：左栏切换场景并管理素材，中栏专注影像，右栏按“堆栈 / 调整 / 输出”组织场景相关参数；无启动门户页和伪步骤条
- 有界内存的“处理前 / 处理后 / 分屏”比较；素材、正式结果与快速预览分别保存缩放、滚动、对比和蒙版状态，返回检查位置时不会重置
- 主窗口内任务反馈：状态栏 2px 真实进度线、处理/取消主按钮、5 秒素材删除撤销、结果失效但保留可看、可复制处理报告
- 输出目录最近 5 个 StarProcessor TIFF 结果；后台使用 libtiff 读取并生成 2400px 只读预览，不把历史文件误认为当前可导出结果
- HTTPS 客户端更新检查：遵循 Windows/macOS 系统代理配置，按平台读取版本清单，流式下载到系统“下载”目录，并在打开安装包前校验文件大小和 SHA-256
...
│   │   ├── Toolbar.h/cpp              # 36px 顶部命令带
│   │   ├── StyleTokens.h/cpp          # 颜色、字阶、间距与全局 QSS
│   │   ├── InspectorWidgets.h/cpp     # 检查器分区、数值滑杆与开关
│   │   ├── ViewStateStore.h/cpp       # 内容级视图状态 LRU
│   │   └── TaskStatusBar.h/cpp        # 单行状态与真实任务进度线
│   ├── update/
│   │   └── UpdateManager.h/cpp        # HTTPS 检查、下载、大小与 SHA-256 校验
│   └── workers/
│       ├── ProcessingWorker.h/cpp     # 正式处理管线后台执行
│       ├── ExportWorker.h/cpp         # 完整分辨率手动导出的可取消后台任务
│       ├── HistoryPreviewWorker.h/cpp # 历史 TIFF 的后台只读预览
...
│   ├── PreviewPanelTests.cpp    # 完整分屏与内容级视图状态测试
│   ├── UiFoundationTests.cpp    # Token、组件、状态栏与 LRU 测试
│   └── ProjectPanelTests.cpp    # 场景、参考帧、撤销与编辑锁测试
├── tools/
...
应用启动后直接进入三栏工作台。左栏顶部在“单张精修、银河星景、深空天体、天地分离、星轨合成、延时序列”之间切换，素材不会因切换场景丢失；中栏显示源照片、结果、分屏、蒙版和灰点采样；右栏只展示当前场景可用的“堆栈 / 调整 / 输出”参数。“处理”按当前门槛激活，处理中原位变为“取消”，导出只在结果签名与当前参数完全一致时开放。

场景会改变素材门槛、参数基线和可见分区。单张精修只要求 1 张 RAW，不进行对齐堆栈；银河和天地运行各自的堆栈流程。深空场景把左栏素材视为 Light，左栏底部同步显示 Dark、Flat、Bias、Dark Flat 摘要，实际路径在堆栈页管理。星轨要求至少 3 张固定机位 RAW；延时以 3/5 帧窗口逐张输出。堆栈算法下方持续显示适用场景、优势与代价，非 Kappa-Sigma 模式自动隐藏无效的 κ 语义。

处理结束后可查看处理前、处理后，或将两张完整图片等比例并排比较；切回某张 RAW 检查后，可用浮动工具条“结果”或 R 键返回。素材、正式结果和快速预览各自保留 fit/manual、缩放、滚动与比较状态。参数、素材或场景改变后，上次成片保留可看但不可导出；新正式任务开始时才释放旧结果。

完成一次正式处理后，应用缓存最长边 2400 px 的线性 RGB16 堆栈和对应天地蒙版。收尾参数停止变化 400 ms 后启动快速预览，过期任务协作取消；上游参数变化要求完整重算。处理期间素材仍可浏览，但导入、删除、排除、参考帧、场景和参数编辑被锁定。状态栏显示真实进度和阶段，完成后输出页提供对齐、光度、筛选、蒙版与耗时报告，并扫描最近 5 个 TIFF 供后台只读预览。

界面结构、视觉变量和离屏截图方法见 [`docs/ui-workflow.md`](docs/ui-workflow.md)。

## 客户端更新

从 v0.9.0 起，应用每天最多静默检查一次更新，也可通过“帮助 → 检查更新”手动执行。更新请求继承操作系统代理/PAC 配置。发现新版本后由用户确认下载；应用不会静默安装。安装包必须来自项目的 HTTPS 下载目录，并同时通过清单声明的文件大小与 SHA-256 校验，才会保存到系统“下载”目录并允许打开。Windows 上如果杀毒软件或索引器短暂阻止原子改名，客户端会复制到唯一文件名并再次校验完整文件，不会覆盖已有安装包。
...
> **注意**：当前处理链路为：RAW AHD 与裁剪高光混合 → 自动镜头横向色差校正 → 对齐 → 帧间光度匹配 → RGB 联动堆栈 → 改机色彩还原 → 线性降噪 → 去雾/自动拉伸 → 基础调色 → 地景细节 → 亮度锐化 → 星点去紫边 → Starless/Stars 缩星 → 导出。未启用的阶段会直接跳过。
...
- **去色边与缩星边界**：去紫边和缩星是两个独立选项；前者只比较星核与星翼的相对色度，不改变半径，也不会把整幅图的紫色去饱和。当前 Starless 是传统 CV 的局部背景近似，不是 AI 去星；缩星强度 40/70/90 分别适合温和、强烈和多数暗弱小星清除。饱和大星和宽光晕默认保持原样，防止形成暗环；衍射芒、单帧曝光拖线、密集星云结节和不同焦段仍应在 100% 比例复核
- **镜头色差边界**：自动校正只处理随画面半径规律变化的横向色差，并以绿色通道为几何基准。它不会处理焦内/焦外轴向色差、亮星宽紫晕、大气色散或真实星色，这些仍由可选去紫边与人工复核负责。当前只接入单张、银河/天地和深空堆栈；星轨与延时序列保持原样
- **自动裁切**：天空对齐后自动裁到所有成功帧的共同有效区域，避免零填充边界造成单帧/少帧接缝；长序列或大位移会相应损失少量画幅
- **统计堆栈性能**：Kappa-Sigma 对每个 RGB 像素的亮度样本排序并迭代裁剪，再用同一保留帧集合合成三个通道；15 张 36 MP Sony RAW 实测完整流程约 30 秒，明显慢于 Average，但 RAM 峰值不随帧数线性增长
