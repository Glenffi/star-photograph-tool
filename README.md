...
> **当前版本**：v0.6.6。核心闭环、Bayer 域深空校准、带自动/手动灰点和连续强度的 BCF 改机色彩还原、天地分离、星轨合成、延时图片序列降噪、自适应 Arcsinh 拉伸、线性多尺度降噪、带中小星去色边的亚像素 Starless/Stars 缩星、低分辨率参数快速预览、轻量 RAW 浏览预览和核心自动化测试已经接入；Windows 仍需实机验证。
